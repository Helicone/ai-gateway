...
impl RouterStore {
    ...
    pub async fn get_all_routers(
        ...
        let res = sqlx::query_as::<_, DBRouterConfig>(
            "SELECT DISTINCT ON (routers.hash) routers.hash, config FROM\
            router_config_versions INNER JOIN routers on router_config_versions.router_id = routers.id ORDER BY routers.hash, router_config_versions.created_at DESC",
        )
        .fetch_all(&self.pool)
        .await
        .map_err(|e| {
